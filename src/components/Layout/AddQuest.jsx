...
const AddTask = ({ sector_id }) => {
  const { add_quest, alert, loading } = useContext(SectorContext);
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      if (questFormData.options.length > 4) {
        ...
        await add_quest(questFormData, sector_id)
          .then(() => {
      ...
      <Container className="pb-5 card-container">
        <Card className="mt-5  px-0">
          <Card.Header style={{ justifyContent: "flex-start" }}>
            <h3 className={`m-0 p-0 mx-3`}>Add Sector Quest</h3>
          </Card.Header>
          {alert && (
          ...
          <Card.Body className="px-4">
            ...
            <Row className="m-0 p-0 mb-4">
              <Col className="d-flex justify-content-between gap-2">
                <CustomButton type="submit" text={"Add Quest"} />
              </Col>
            </Row>
          </Card.Body>
        </Card>
